...
def test_phone_never_in_message(db):
    ...
    for lang in ("hi", "en"):
        ...
        wa = render_wa("template_1", "Ramesh", "Sampletown CSP", "Main Rd", "9876543210")
        sms = render_sms("Ramesh", "Sampletown CSP", "9876543210", "Main Rd")
...
def test_both_languages_render():
    for lang in ("hi", "en"):
        wa = render_wa("template_3", "Ramesh", "Sampletown CSP", "Main Road", lang=lang)
        sms = render_sms("Ramesh", "Sampletown CSP", branch_address="Main Road", lang=lang)
        assert "Ramesh" in wa and "Sampletown CSP" in wa and "Main Road" in wa
        assert "Ramesh" in sms and "Main Road" in sms
    # english is the readable GSM-7 SMS; sanity-check it stays a short message
    en_sms = render_sms("Ramesh", "Sampletown CSP", branch_address="Main Road", lang="en")
