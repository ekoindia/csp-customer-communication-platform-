...
JSON sample (Tamkuhi Raj / Testpur / Kushinagar) — not extracted from any
real customer scan.
"""
...
def test_clean_village_strips_taluka_and_address_bleed():
    assert _clean_village("Testpur AH") == "Testpur"
    assert _clean_village("Testpur DU") == "Testpur"
    assert _clean_village("Testpur") == "Testpur"
    assert _clean_village("Raj AHIRAUI").lower() != "raj"        # taluka word dropped
    assert _clean_village("Testpur 800001") == "Testpur"        # digit fragment dropped
...
def test_relation_prefix_splits_all_three_fields():
    tail = "Tamkuhi Raj Testpur S/O NOOR ALAM VILL-TESTPUR POST-SAMPLETOWN DIST-KUSHINAG 800001"
    r = _split_trailing_fields(tail)
    assert r["taluka"] == "Tamkuhi Raj"
    assert r["village"] == "Testpur"
    assert r["address"].startswith("S/O")
    assert "800001" in r["address"]
...
def test_village_not_blank_when_no_relation_prefix():
    ...
    tail = "Tamkuhi Raj Testpur VILL-TESTPUR POST-SAMPLETOWN DIST-KUSHINAGAR 800001"
    r = _split_trailing_fields(tail)
    assert r["village"] == "Testpur"
    assert r["village"] != ""
...
def test_address_never_duplicates_taluka():
    ...
    tail = "Tamkuhi Raj Sampletown W/O NOOR ALAM VILL-TESTPUR; POST-SAMPLETOWN DIST-KUSHINAG"
    r = _split_trailing_fields(tail)
    assert r["address"] != r["taluka"]
    assert r["taluka"] == "Tamkuhi Raj"
    assert r["village"] == "Sampletown"
...
def test_ocr_mangled_markers_still_split():
    ...
    tail = "Tamkuhi Raj Testpur S/0 NOOR ALAM VILL-TESTPUR P0ST-SAMPLETOWN 800001"
    r = _split_trailing_fields(tail)
    assert r["village"] == "Testpur"
...
def test_pin_code_is_last_resort_address_anchor():
    ...
    tail = "Tamkuhi Raj Testpur TESTPUR KUSHINAGAR 800001"
    r = _split_trailing_fields(tail)
    assert r["taluka"] == "Tamkuhi Raj"
    assert r["village"] == "Testpur"
    assert "800001" in r["address"]
...
def test_village_never_repeats_taluka():
    ...
    tail = "Tamkuhi Raj Tamkuhi Raj Testpur VILL-TESTPUR 800001"
    r = _split_trailing_fields(tail)
    assert r["taluka"] == "Tamkuhi Raj"
